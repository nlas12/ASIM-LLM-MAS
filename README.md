...
- Period-by-period portfolio values
- Trade execution log
- Performance metrics (Sharpe ratio, max drawdown, etc.)
- Benchmarks vs MSCI World Index

## Implementation Details

### LLM Configuration

All agents use **Google Gemini 2.5 Flash Lite** with:
- **Screening & Analysis:** Temperature = 0.5 (balanced reasoning)
- **Decision Making:** Temperature = 0.1 (deterministic orders)
- **Retry Logic:** MAX_RETRIES=2, 2-second delays
- **Structured Output:** Pydantic schema validation

### Memory System

Each agent maintains persistent memory:
- **Trade History:** All executed trades with prices and reasoning
- **Position Tracking:** Entry prices, cost basis, unrealized P&L, periods held
- **Period Returns:** Portfolio value and return % per period
- **Rationales:** Quarterly decision summaries for context

Memory context is injected into all LLM prompts to enable learning and consistency.

### Execution Flow

1. **Data Loading** (`data_loader.py`): Fetch NASDAQ-100 fundamentals & prices
2. **Independent Screening** (Pass 1): Each persona identifies candidates
3. **Deep Analysis** (Pass 1): Financial metrics analysis + conviction levels
4. **Independent Decision** (Pass 1): Trade order generation
5. **Decision Summary** (Step 4): Summarize all agents' proposals
6. **Peer-Informed Re-Analyses** (Pass 2): Re-analyze with peer context
7. **Revised Decisions** (Pass 2): Updated trade orders
8. **Coordination** (Step 7): Aggregate via selected mechanism
9. **Execution** (Step 8): Execute final orders, update memories

### Key Design Patterns

- **Tool-Free LLM Calls:** Direct JSON prompting (no function calling overhead)
- **Stateful Agents:** Memory system provides context across 20+ quarter periods
- **Retry Resilience:** Fallback chains for LLM failures
- **Parallel Execution:** ThreadPoolExecutor for multi-agent passes
- **Comprehensive Logging:** All LLM calls logged for auditability

## Extending the System

### Adding a New Persona

1. Define the system prompt in [src/personas.py](src/personas.py):

```python
MY_PERSONA_PROMPT = """
## Role
You are MyInvestor, known for...
...
PERSONAS["my_investor"] = MY_PERSONA_PROMPT
```

2. Run with your persona:

```bash
python src/run_experiment.py --personas my_investor --start 2014-01-01 --end 2023-12-31
```

### Modifying Coordination Mechanisms

Edit [src/multi_agent_pipeline.py](src/multi_agent_pipeline.py) to add a new `CoordinationMechanism`:

```python
class MyCoordination(CoordinationMechanism):
    name = "my_mechanism"
    
    def aggregate(self, revised_states, price_data, portfolio):
        # Implement your aggregation logic
        return final_orders  # list[TradeOrder]

COORDINATION_MECHANISMS["my_mechanism"] = MyCoordination()
```
